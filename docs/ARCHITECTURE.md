...
| 2026-05-23 | "can you make this a powerpoint?"             | Added an **editable PowerPoint** build (`PITCH.pptx`): `md2pptx.py` rewrites the beamer-only LaTeX (`\alert`, `\vspace`, `\begin{center}`, size macros) into plain Markdown and folds image captions so PowerPoint keeps them on one slide; `render.sh` rasterizes the deck's diagrams to PNG (pptx can't embed PDF) and runs pandoc → pptx. Single canonical source (`PITCH.md`); 14 slides verified. |
